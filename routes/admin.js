...
router.get('/sale', function (req, res) {
    ...
    const medicineQuery = "SELECT * from medicine_information";
    const medicineName = []
    ...
    function generateRandomNumber(length) {
      ...
      db.getData(medicineQuery, null, function(rows){
        rows.map((doc) => {
            medicineName.push(doc.Medicine_Name);
        })
      })
    ...
    db.getData(query, null, function (rows) {
        var data = {
            'batch': rows,
            user: req.session.loggedUser,
            'invoice_num': generateRandomNumber(7),
